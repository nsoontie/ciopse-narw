...
mmdd=0109 # Replace with the month/day to process. Can be * to process all months/days in a year or 01* to process all days in January.
HH=00 # Only the 00 hour files because 3D are daily.

mkdir -p ${OUT_DIR}/${YEAR}
mkdir -p ${OUT_DIR}/tmp
files=$(ssh gpsc-vis2 "ls ${SRC_DIR}/${YEAR}${mmdd}${HH}*")
